/*
想法:这个采用两个函数不改变大区间两端的值,这样能直接回溯,这两个大区间直接保护两个小区间当中难以找到的端点处
反思:最主要这个while循环里面还有两个while循环导致指针乱移动,数组越界,造成这个原因是循环判断条件懒省事,直接写的不相等而不是大于小于值
改了2小时,学会了调试才改出来,下次循环条件一定要严格定死,有更小范围就不用大范围,大范围可能包含隐秘的漏洞
